...
- Mock AI provider, prepared for OpenAI/Gemini integration
...
| `AI_PROVIDER` | `mock` | Current implementation uses a mock provider. |
...
The current AI embedding implementation is still mock-based until a real provider is integrated.
...
- AI provider is currently mock/heuristic. Replace `backend/src/ai/ai.service.ts` with an OpenAI/Gemini adapter for production-quality parsing, screening, question generation, and embeddings.
