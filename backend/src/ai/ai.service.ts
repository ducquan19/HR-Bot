...
const parsedCvJsonSchema = {
  ...
  properties: {
    firstName: {
      type: "string",
      description: "Candidate given name if available.",
    },
    lastName: {
      type: "string",
      description: "Candidate family name or remaining full name if available.",
    },
    email: {
      type: "string",
      description: "Candidate email address if available.",
    },
    phone: {
      type: "string",
      description: "Candidate phone number if available.",
    },
    education: {
      type: "array",
      items: { type: "string" },
      description: "Education entries as concise strings.",
    },
    skills: {
      type: "array",
      items: { type: "string" },
      description:
        "Technical and professional skills, normalized and deduplicated.",
    },
    experienceYears: {
      type: "number",
    ...
    experiences: {
      ...
      items: {
        ...
        properties: {
          ...
          startDate: {
            type: "string",
            description: "Original or ISO-like start date if available.",
          },
          endDate: {
            type: "string",
            description: "Original or ISO-like end date if available.",
          },
    ...
    projects: {
      type: "array",
      items: { type: "string" },
      description: "Projects as concise descriptions.",
    },
    certifications: {
      type: "array",
      items: { type: "string" },
    ...
    languages: {
      type: "array",
      items: { type: "string" },
      description: "Human languages and proficiency if available.",
    },
    summary: {
      type: "string",
      description: "Short recruiter-friendly profile summary.",
    },
  },
  required: [
    "education",
...
const screeningResultJsonSchema = {
  ...
  properties: {
    ...
    skillScore: {
      type: "number",
      description: "Skill matching score from 0 to 100.",
    },
    educationScore: {
      type: "number",
    ...
    experienceScore: {
      type: "number",
      description: "Experience fit score from 0 to 100.",
    },
    recommendation: {
      type: "string",
    ...
    strengths: {
      type: "array",
      items: { type: "string" },
      description: "Evidence-based strengths from the CV against the JD.",
    },
    weaknesses: {
      type: "array",
      items: { type: "string" },
      description: "Evidence-based limitations or concerns.",
    },
    missingSkills: {
      type: "array",
      items: { type: "string" },
      description: "Required or important skills not found in the CV.",
    },
    explanation: {
      type: "string",
      description:
        "Concise, transparent explanation of the score and recommendation.",
    },
  },
  required: [
    "overallScore",
...
export class AiService implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    if (nodeEnv === "production" && (provider !== "gemini" || !geminiApiKey)) {
      throw new Error(
        "Production requires AI_PROVIDER=gemini and GEMINI_API_KEY for real AI extraction and screening",
      );
    }

    if (provider !== "gemini") {
      this.logger.warn(
        "AI_PROVIDER is not gemini; CV extraction, screening and embeddings are using local mock providers",
      );
    }
  }
  ...
  private async parseCvWithGemini(rawText: string): Promise<ParsedCv> {
      ...
      `${endpoint}/models/${encodeURIComponent(model)}:generateContent`,
      {
        ...
        body: JSON.stringify({
          contents: [
            {
              role: "user",
              parts: [{ text: this.buildCvExtractionPrompt(rawText) }],
            },
          ],
          generationConfig: {
            temperature: 0.1,
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Gemini CV extraction failed with ${response.status}: ${errorText.slice(0, 500)}`,
      );
    }
  ...
  private async parseCvWithMock(rawText: string): Promise<ParsedCv> {
    ...
    const skills = knownSkills.filter((s) =>
      new RegExp(s.replace(".", "\\."), "i").test(normalized),
    );
  ...
  private async screenCandidateWithGemini(input: ScreeningInput) {
      ...
      `${endpoint}/models/${encodeURIComponent(model)}:generateContent`,
      {
        ...
        body: JSON.stringify({
          contents: [
            {
              role: "user",
              parts: [{ text: this.buildScreeningPrompt(input) }],
            },
          ],
          generationConfig: {
            temperature: 0.15,
    ...
    if (!response.ok) {
      const errorText = await response.text().catch(() => "");
      throw new Error(
        `Gemini candidate screening failed with ${response.status}: ${errorText.slice(0, 500)}`,
      );
    }
    ...
    const text = payload?.candidates?.[0]?.content?.parts
      ...
      .join("\n");
    if (!text)
      throw new Error("Gemini candidate screening returned no text content");
  ...
  private buildScreeningPrompt(input: ScreeningInput) {
    return [
      ...
      JSON.stringify(
        (input.positionSkills ?? []).map((positionSkill) => ({
  ...
  private async screenCandidateWithMock(input: ScreeningInput) {
    const requiredSkills =
      input.positionSkills?.map((ps) => ps.skill.name) ?? [];
    ...
    const experienceScore = Math.min(
      100,
      (input.parsedCv.experienceYears / Math.max(requiredYears, 1)) * 100,
    );
  ...
  async generateInterviewQuestions(input: ScreeningInput) {
    const missing =
      input.positionSkills
        ?.map((ps) => ps.skill.name)
        .filter((s) => !input.parsedCv.skills.includes(s)) ?? [];
    ...
    if (this.genai) {
      try {
        ...
        const generated = JSON.parse(response.text || "[]") as Array<{
          category: string;
          question: string;
        }>;
        ...
        throw new Error(
          "AI API Quota exceeded or unavailable. Cannot generate interview questions at this time.",
        );
      }
    }
    ...
    throw new Error(
      "AI API is not configured. Cannot generate interview questions.",
    );
  }
    ...
    if (this.genai) {
      try {
        ...
        return {
          score: 0,
          feedback:
            "Hệ thống AI đang quá tải hoặc không khả dụng. Vui lòng thử lại sau hoặc để HR đánh giá thủ công.",
        };
      }
    }
  ...
  private async embedWithGemini(text: string, purpose: "query" | "document") {
    ...
    const response = await fetch(
      `${endpoint}/models/${encodeURIComponent(model)}:embedContent`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        ...
        body: JSON.stringify({
          content: {
            parts: [{ text: this.formatEmbeddingText(text, purpose) }],
          },
    ...
    if (!response.ok) {
      const errorText = await response.text().catch(() => "");
      throw new Error(
        `Gemini embedding failed with ${response.status}: ${errorText.slice(0, 500)}`,
      );
    }
    ...
    if (!Array.isArray(values) || values.length !== dimensions) {
      throw new Error(
        `Gemini embedding returned invalid vector length: ${Array.isArray(values) ? values.length : "none"}`,
      );
    }
  ...
  private formatEmbeddingText(text: string, purpose: "query" | "document") {
    ...
    if (purpose === "document")
      return `title: Candidate CV profile | text: ${normalized}`;
  ...
  private extractExperienceYears(text: string): number {
    const matches = [
      ...text.matchAll(/(\d+(?:\.\d+)?)\s*(?:\+\s*)?(years?|yrs?|năm)/gi),
    ].map((m) => Number(m[1]));
